    ...
    <div className="wrap">
      ...
      <header className="hero">
        <div className="herocopy">
          <p className="iam-kicker">AI POLICY STUDIO / SERVICE-SIDE TOOL</p>
          <h1 className="herohead">
            <span className="nowrap">AI Policy Studio</span>
          </h1>
          <p className="lead">{lang === "ja" ? "サービス導入時に、目的と現在の取得項目をAIが読み、許可済みClaimだけを使ったPolicy案へ変換する導入担当者向けツールです。" : "A service-side tool that turns purposes and current data requests into a constrained Policy proposal during onboarding."}</p>
          <a className="cta" href="#demo">{lang === "ja" ? "Policyを分析する" : "Analyze a policy"} ↓</a>
        </div>
      </header>
      ...
      <section id="demo" className="demo">
        ...
        <p className="demowho">{t.demoWho}</p>
        ...
        <>
          ...
          {verify && (
            <section className="stepcard">
              ...
              {(verify.status === "VALID" || verify.status === "REVOKED") && (
                <div className="payoff">
                  <p className="payoff-title">{t.payoffTitle}</p>
                  <p className="payoff-body">{t.payoffBody}</p>
                </div>
              )}
              <details className="tech">
