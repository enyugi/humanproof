...
| D-032 | 採用 | **IAMme**を長期プロダクト／上位ブランド、**HumanProof**をAI HACK 2026向けPoC名・IAMmeのNOWに位置する実装断面とする。公開サイトは `/`=IAMme LP、`/demo`=HumanProof Guided Demo、`/studio`=AI Policy Studioへ分離する | ユーザーとの命名決定 + 2026-08-13リリース体験監査 | HumanProofがIAMmeを上書きする名称ドリフトと、LP・一般利用者デモ・導入者向け設定UIの役割混同を防ぐ。詳細は `RELEASE_EXPERIENCE_CANON.md` |
| D-033 | 採用 | `/demo`は、利用者がIAMmeデモアカウントを作成し、Demo Trusted Issuerの属性元確認を操作（元確認のみ模擬）してから、架空EC `NIGHT SCREEN`で保存済みPolicyに基づく2 Proofを共有し、実APIの`VALID → REVOKED`まで完走する5段階のGuided Demoとする | ユーザー体験レビュー + 2026-08-13最終改修 | 「認証済み属性がどこから来たか」を省略せず、Policy Studioを利用者画面と誤認させない。AIは導入時、購入時は保存済みPolicyという責務を体験上も固定する |
