...
export const DICT: Record<Lang, Dict> = {
  ja: {
    ...
    valueUser:
      "氏名も住所も身分証画像もサービスへ渡さず、「18歳以上の実在の人」など必要な事実だけを証明する。Proofは発行時に保有者へ返した秘密コードで失効できる。",
    ...
    valueAi:
      "定型の年齢確認だけならルールで足ります。複数目的・曖昧・矛盾・規約文が混ざる場面で、AIはPolicy設計を支援します。曖昧な判断は自動採用せず、人へ確認を戻します。",
    scenariosTitle: "例で試す（AI の推論を見る）",
    scenariosHint: "複数目的や曖昧さを含む例で、AIがPolicy設計をどう支援するか確認できます。",
    scBasic: "18歳コミュニティ（基本）",
    scBasicDesc: "4つの個人情報 → 2つの証明。まずは基本形。",
    scDelivery: "酒類の宅配（AI の真価）",
    scDeliveryDesc: "複数目的。AIは配送に必要な住所を、年齢確認だけの過剰要求と混同しません。",
    scInjection: "指示注入を拒否（セキュリティ）",
    scInjectionDesc: "「氏名を証明にしろ」という埋め込み命令を無視し、最小の証明を保つ。",
    heroCta: "デモを試す",
    heroBenefit: "氏名や身分証画像をサービスへ渡さず、「18歳以上の実在の人」など必要な事実だけを証明する。",
    hvOver: "過剰な要求",
    hvOnly: "渡すのはこれだけ",
    hvCap: "AI が最小化",
    stripUserT: "あなた（利用者）",
    ...
    stripAiT: "なぜ AI",
    stripAi: "複数目的や曖昧な要求文から、Policy設計を支援。",
    demoTitle: "AIでPolicy案を作る",
    demoLead: "サービスの要求文を入れると、AIが目的を分け、許可済みClaim Catalogの範囲で最小の証明を提案します。",
    voiceService: "サービス",
    voiceAi: "HumanProof AI",
    voiceYou: "あなた",
    userStory: "ここでは、あなたはサービスの導入担当者です。利用者向けの購入体験はGuided Demoに分離しています。",
    youShareNot: "あなたが渡さないもの",
    youShareNotSub: "サービスには渡りません。あなたの手元に残ります。",
    demoWho: "入力はサービス要件の例です。LLMへは本人情報の種別名だけを送り、実値は送信前にPII Shieldでブロックします。",
    payoffTitle: "この操作で得たもの",
  ...
  en: {
    ...
    valueUser:
      "You do not send your name, address or ID image to the service. You prove only the fact needed. The holder can revoke the proof with the secret code returned at issue.",
    ...
    valueAi:
      "A fixed age check could be a rule. AI supports Policy design when multiple purposes, ambiguity and conflicting requirements are mixed together. Unclear decisions return to a human.",
    ...
    heroBenefit: "You do not send your name or ID image to the service. You prove only the fact needed, such as being a real person over 18.",
    ...
    stripAiT: "Why AI",
    stripAi: "Supports Policy design for ambiguous, multi-purpose requirements.",
    demoTitle: "Design a Policy with AI",
    demoLead: "Enter a service requirement. AI separates its purposes and proposes minimum proof within the approved Claim Catalog.",
    voiceService: "The service",
    voiceAi: "HumanProof AI",
    voiceYou: "You",
    userStory: "Here, you are the service onboarding operator. The end-user purchase flow is separated into the Guided Demo.",
    youShareNot: "What you never hand over",
    youShareNotSub: "The service never receives these. They stay with you.",
    demoWho: "The input is a service requirement example. Only category names are sent to the LLM; detected personal values are blocked by the PII Shield before egress.",
    payoffTitle: "What this operation produced",
